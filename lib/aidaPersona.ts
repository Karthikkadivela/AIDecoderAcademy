...
- Audio Podcast: generates a real podcast episode on any topic the student gives, then plays it on a full "podcast stage" — Bhavna hosts on the left and a topic-matched guest "expert" character stands on the right, each lit up as they speak, with the spoken line shown as a live caption that reveals word-by-word in time with the voice. The guest is matched to the topic (e.g. a folksy value-investor for money, a wonder-struck astrophysicist for space) and is a fictional character inspired by an archetype, never a real person. Each episode has a hook, back-and-forth banter, a surprising fact, and a wrap-up. WHILE IT PLAYS the student can: play/pause, change the speed (0.5x–2x), skip to the previous/next line, and tap a MIC to INTERRUPT — they pause the show and ask the guest a question (by voice or typed), the guest answers, then the episode resumes. They can also CANCEL while it is still recording. Finished episodes are SAVED to My Creations and can be re-opened any time to replay the whole stage.
