...
// Steel-and-cyan signature (matches the AIDA panel chrome).
...
export function VoiceOrb({ state, amplitude = 0, size = 160, className }: Props) {
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      halo.addColorStop(0, `rgba(${CYAN_MID}, ${0.28 + rayFlare * 0.25})`);
      halo.addColorStop(0.5, `rgba(${CYAN_DEEP}, 0.10)`);
      halo.addColorStop(1, `rgba(${CYAN_DEEP}, 0)`);
      ctx!.fillStyle = halo;
      ...
      for (let i = 0; i < TICKS; i++) {
        ...
        ctx!.strokeStyle = `rgba(${CYAN_CORE}, ${dialAlpha})`;
        ctx!.lineWidth = 1.5;
        ctx!.stroke();
      }
      ...
      if (st === "listening" && !reduced) {
        ...
        ctx!.strokeStyle = `rgba(${CYAN_MID}, ${0.35 * (1 - ripple)})`;
        ctx!.lineWidth = 2;
        ctx!.stroke();
      }
      ...
      starPath(r * (1.7 + rayFlare * 1.3), r * 0.42, rot);
      const flareGrad = ctx!.createRadialGradient(cx, cy, 0, cx, cy, r * 2.6);
      flareGrad.addColorStop(0, `rgba(${CYAN_CORE}, ${0.85})`);
      flareGrad.addColorStop(0.6, `rgba(${CYAN_MID}, ${0.35 + rayFlare * 0.3})`);
      flareGrad.addColorStop(1, `rgba(${CYAN_MID}, 0)`);
      ctx!.fillStyle = flareGrad;
      ctx!.fill();

      // Solid core star
      starPath(r, r * 0.46, rot);
      const coreGrad = ctx!.createRadialGradient(
        cx - r * 0.3, cy - r * 0.3, r * 0.1, cx, cy, r,
      );
      coreGrad.addColorStop(0, "rgba(255,255,255,0.95)");
      coreGrad.addColorStop(0.4, `rgba(${CYAN_CORE}, 0.95)`);
      coreGrad.addColorStop(1, `rgba(${CYAN_DEEP}, 0.9)`);
      ...
      for (let i = sparks.length - 1; i >= 0; i--) {
        ...
        ctx!.fillStyle = `rgba(${CYAN_CORE}, ${0.9 * k})`;
        ctx!.fill();
      }
    }
