...
const TILES = [
  { key:"light",       src:"/classroom/physics/LIGHT.png",        num:1, locked:false,
    style:{ top:"calc(50% - 362px)", left:"calc(47% - 150px)" } },
  { key:"human_eye",   src:"/classroom/physics/HUMAN EYE.png",    num:2, locked:true,
    style:{ top:"calc(50% - 162px)", left:"calc(47% + 131px)" } },
  { key:"electricity", src:"/classroom/physics/ELECTRICITY.png",  num:3, locked:true,
    style:{ top:"calc(50% + 163px)", left:"calc(47% + 25px)" } },
  { key:"magnetic",    src:"/classroom/physics/MAGNETIC.png",     num:4, locked:true,
    style:{ top:"calc(50% + 163px)", left:"calc(47% - 315px)" } },
  { key:"sources",     src:"/classroom/physics/SOURCES.png",      num:5, locked:true,
    ...
    <motion.div className="flex flex-col items-center flex-1"
      ...
      <div className="rounded-full flex items-center justify-center"
        style={{ width:meta.avatar, height:meta.avatar,
          background:isMe?"rgba(14,165,233,0.12)":"rgba(255,255,255,0.88)",
      ...
      <div className="font-black text-center truncate mt-0.5"
        style={{ fontSize:10, color:isMe?"#0EA5E9":"#1a1a2e", maxWidth:64 }}>
        {isMe?"You":entry.display_name.split(" ")[0]}
      </div>
      <div className="font-black" style={{ fontSize:10, color:"#0EA5E9", marginTop:1 }}>
...
function LeaderboardRow({ entry, index }: { entry:LeaderboardEntry; index:number }) {
  ...
  return (
    <motion.div className="flex items-center gap-2 px-2.5 py-2 rounded-xl"
      ...
      style={{ background:isMe?"rgba(14,165,233,0.08)":"rgba(0,0,0,0.02)",
      ...
      <div className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center"
        style={{ background:isMe?"rgba(14,165,233,0.1)":"rgba(0,0,0,0.05)", fontSize:13 }}>
        {entry.avatar_emoji||"🧑‍💻"}
      </div>
      <div className="flex-1 min-w-0">
        <div className="font-bold truncate" style={{ fontSize:11, color:isMe?"#0EA5E9":"#1a1a2e", lineHeight:1 }}>
          {isMe?"You":entry.display_name.split(" ")[0]}
        </div>
        <div style={{ fontSize:9, color:"#bbb", lineHeight:1, marginTop:2 }}>Lv {entry.level}</div>
      </div>
      <div className="font-black flex-shrink-0" style={{ fontSize:11, color:isMe?"#0EA5E9":"#333" }}>
        {entry.xp.toLocaleString()}
      </div>
    </motion.div>
  );
...
export function PhysicsChapterMapPage({ onChapterSelect, onBack }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const physicsChaps = (chapData.chapters as Chapter[]).filter(
        c => c.subject === "Physics" && c.chapter_number < 90
      );
      setChapters(physicsChaps);
      setLbData(lb);
      setLoading(false);
    });
  }, []);
  ...
  const handleClick = (num: number, locked: boolean) => {
    ...
    const ch = chapters.find(c => c.chapter_number === num) ?? {
      ...
      subject:        "Physics",
      chapter_number: num,
      chapter_title:  CHAPTER_TITLES[num] ?? `Chapter ${num}`,
      grade:          10,
      board:          "CBSE",
  ...
  const physicsChapters = chapters.filter(c => c.chapter_number <= 5);
  const progressPct     = physicsChapters.length / 5;
  ...
  return (
    <div className="relative overflow-hidden"
      ...
      <img src="/classroom/physics/BACKGROUND.png" alt="" aria-hidden draggable={false}
      ...
      <header className="relative flex items-center px-6 py-3.5 gap-4 flex-shrink-0"
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl"
          ...
          <span className="text-xs font-bold" style={{ color:"#0EA5E9" }}>Board: CBSE</span>
        </div>
      </header>
      ...
      <div className="relative" style={{ height:"calc(100dvh - 57px)", zIndex:10 }}>
        ...
        <motion.div initial={{ opacity:0, scale:0.8 }} animate={{ opacity:1, scale:1 }}
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/classroom/physics/mid_term_exam.png" alt="Integrated Exam Practice"
            ...
            <div className="relative rounded-xl overflow-hidden"
              style={{
                ...
                boxShadow: tile.locked
                  ...
                  : "0 6px 28px rgba(15,28,77,0.18), 0 0 0 2px rgba(14,165,233,0.3)",
              }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <motion.div initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }}
          ...
          <div className="flex items-center gap-4 mb-3">
            <ProgressRing pct={progressPct} color="#0EA5E9" />
            <div className="space-y-2 flex-1">
              {[
                { label:"Chapters Completed", value:`${physicsChapters.length}/5` },
        ...
        <motion.div initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }}
          ...
          <div className="flex-shrink-0 flex items-center gap-2 px-3 py-2.5"
            style={{ background:"linear-gradient(135deg,rgba(14,165,233,0.10),rgba(0,212,255,0.05))",
            ...
            <>
              {podium.length === 3 && (
                <div className="flex-shrink-0 flex items-end gap-1 px-3 pt-4 pb-1"
                  style={{ background:"linear-gradient(180deg,rgba(14,165,233,0.04),transparent)" }}>
