...
export function ObjectiveSubmissionPanel({
  ...
  async function handleValidate() {
    ...
    if (isObj2 || isObj4 || isObj9) {
      if (whiteboardImages.length >= 3) {
        const n = whiteboardImages.length;
        mediaToUse = [whiteboardImages[n - 3].url, whiteboardImages[n - 2].url, whiteboardImages[n - 1].url];
      }
    } else if (isObj3) {
      if (whiteboardImages.length >= 2) {
        const n = whiteboardImages.length;
        mediaToUse = [whiteboardImages[n - 2].url, whiteboardImages[n - 1].url];
      }
    } else if (isObj5 && whiteboardAudios.length > 0) {
      mediaToUse = [whiteboardAudios[whiteboardAudios.length - 1].url];
    } else if (isObj8 && whiteboardAudios.length >= 3) {
      const n = whiteboardAudios.length;
      mediaToUse = [whiteboardAudios[n - 3].url, whiteboardAudios[n - 2].url, whiteboardAudios[n - 1].url];
...
function ReadyView({
  pending, isObj6, isObj5, isObj8, whiteboardImageCount, whiteboardAudioCount,
}: {
