...
export function WorksheetPopup({
  ...
  // a DOCX dropped in chat and writing the result to the :draft localStorage key.
  // Re-reads from localStorage (same as the open-effect) so the form always
  // reflects what was actually saved — no data passed through the event itself.
  useEffect(() => {
    if (!schema) return;
